...
if [ -n "${TMPDIR-/tmp}" ]
	then
	JVM_ARGS="$JVM_ARGS -Djava.io.tmpdir=\"${TMPDIR-/tmp}\""
elif [ -n "$TMP" ]
	then
...
echo "java $JVM_ARGS $LOGGING -cp "$SHACL_CP" org.topbraid.shacl.tools.Validate "$@" "
